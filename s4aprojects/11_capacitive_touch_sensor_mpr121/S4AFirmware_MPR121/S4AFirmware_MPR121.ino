// MODIFIED: 
// - support for MPR121 capacitive touchboard on Pin A4 and A5
// the touched pin number is provided on the Analog4 value in S4A
...
void sendSensorValues()
{
  ...
  {
    if(sensorIndex == ANALOG_PIN_TOUCH) {
      // custom handling for the analog input
      // being sent to Scratch
      int touched = getCurrentTouched();
      int touchedNumber = 1023;
      if(touched != 0) {
        touchedNumber = 0;
        for (uint8_t i=0; i<12; i++) {
          if(touched & _BV(i)) {
      ...
      sensorValues[sensorIndex] = touchedNumber;
    } else {
      for (byte p = 0; p < 5; p++) {
