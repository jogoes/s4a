// MODIFIED: support for HX1838 IR receiver
...
// the pin SIG is connected to
const int RECV_PIN = 11;
// the pin the pressed button code can be read from
const int ANALOG_INPUT_PIN = 3;
...
int getButtonCode() {
  ...
  if (irrecv.decode(&results)) {
    ...
    // send the last button code for at least 100ms
    // in order to prevent jumping of the code
...
void sendSensorValues()
{
  ...
  {
    if(sensorIndex == ANALOG_INPUT_PIN) {
      // custom handling for the input to which
      // we send the button code
      sensorValues[sensorIndex] = getButtonCode();
    } else {
      for (byte p = 0; p < 5; p++) {
