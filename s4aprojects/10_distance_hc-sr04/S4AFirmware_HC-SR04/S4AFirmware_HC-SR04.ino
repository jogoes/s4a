// MODIFIED: support for HC-SR04 ultrasonic sensor connected on pin 11 (TRIG) and pin 3 (ECHO)
// output to Scratch on Analog 3
...
// the pin the HC-SR04 echo pin is connected to
const int ECHO_PIN = 3;
// the pin the HC-SR04 trigger pin is connected to
const int TRIGGER_PIN = 11;
// the analog pin in Scratch the result is sent to
const int ANALOG_PIN_DISTANCE = 3;
...
// function returning the measured distance in cm
long getDistance() {
...
void sendSensorValues()
{
  ...
  {
    if(sensorIndex == ANALOG_PIN_DISTANCE) {
      // custom handling for the input pin we are sending
      // the distance value to
      sensorValues[sensorIndex] = getDistance();
    } else {
      for (byte p = 0; p < 5; p++) {
