// (JGoebel) support for ADXL345 Digital Accelerometer
// output to Scratch on Analog 3 (X axis) and Analog 4 (Y axis)
// 
// connect the ADXL 345 as follows to the Arduino:
// VIN -> 5V
// GND -> GND
// SDA -> SDA
// SCL -> SCL
...
// the analog input where the angle in the X/Y plane is sent to S4A
const int ANALOG_INPUT_X_ACCEL = 3;
// the analog input where the angle in the Y/Z plane is sent to S4A
const int ANALOG_INPUT_Y_ACCEL = 4;
...
// maps the provided acceleration value to an angle between 0 and 180 degrees
// a value range of -90 to 90 would be more appropriate here but S4A only supports
// positive values for the analog input values
unsigned int toAngle(float accel) {
  return min(max(accel, -10) + 10, 20) * 9;
}
...
void sendSensorValues()
{
  ...
  {
      if(sensorIndex == ANALOG_INPUT_X_ACCEL) {
        sensorValues[sensorIndex] = toAngle(event.acceleration.x);
      } else if (sensorIndex == ANALOG_INPUT_Y_ACCEL) {
        sensorValues[sensorIndex] = toAngle(event.acceleration.y);
      } else {
        for (byte p = 0; p < 5; p++) {
