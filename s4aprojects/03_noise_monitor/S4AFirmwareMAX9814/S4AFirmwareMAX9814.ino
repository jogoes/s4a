// (JGoebel) MODIFIED: 
// - support for microphone amplifier MAX 9814
...
// the Arduino pin number on which the OUT pin of the amplifier is connected to
const int ANALOG_INPUT_AMPLIFIER = 5;
...
// get the sample value from the amplifier
int getSampleValue() {
   int sample;
...
void sendSensorValues()
{
  ...
  {
    if(sensorIndex == ANALOG_INPUT_AMPLIFIER) {
      // read the sample value from the amplifier
      sensorValues[sensorIndex] = getSampleValue();
    } else {
      for (byte p = 0; p < 5; p++) {
